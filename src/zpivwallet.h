...
#ifndef SMRT_ZSMRTWALLET_H
#define SMRT_ZSMRTWALLET_H
...
public:
    CzSMRTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSMRT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSMRT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SMRT_ZSMRTWALLET_H
