...
public:
    ...
    bool WriteZSMRTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSMRTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSMRTSeed_deprecated(uint256& seed);
    bool EraseZSMRTSeed();
    bool EraseZSMRTSeed_deprecated();

    bool WriteZSMRTCount(const uint32_t& nCount);
    bool ReadZSMRTCount(uint32_t& nCount);
