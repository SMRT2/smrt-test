...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SMRT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSMRT send
enum ZerocoinSpendStatus {
    ZSMRT_SPEND_OKAY = 0,                            // No error
    ZSMRT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSMRT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSMRT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSMRT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSMRT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSMRT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSMRT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSMRT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSMRT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSMRT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSMRT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSMRT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSMRT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSMRT_SPENT_USED_ZSMRT = 14,                      // Coin has already been spend
    ZSMRT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSMRT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSMRTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzSMRTWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSMRTTracker> zpivTracker;
    ...
    void setZWallet(CzSMRTWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzSMRTTracker>(new CzSMRTTracker(strWalletFile));
    }

    CzSMRTWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSMRT reset */
    boost::signals2::signal<void()> NotifyzSMRTReset;
