...
CzSMRTWallet::CzSMRTWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSMRTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSMRTSeed_deprecated()) {
                    LogPrintf("%s: Updated zSMRT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSMRTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSMRTCount(nCountLastUsed);
    else if (!walletdb.ReadZSMRTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSMRTWallet::Lock()
{
    seedMaster = 0;
}

void CzSMRTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSMRTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSMRT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSMRTWallet::LoadMintPoolFromDB()
{
...
void CzSMRTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSMRTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSMRTWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSMRTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSMRT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSMRTWallet::SeedToZSMRT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSMRTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSMRTWallet::GenerateDeterministicZSMRT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSMRTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSMRTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
