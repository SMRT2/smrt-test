...
CzSMRTTracker::CzSMRTTracker(std::string strWalletFile)
{
...
CzSMRTTracker::~CzSMRTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSMRTTracker::Init()
{
...
bool CzSMRTTracker::Archive(CMintMeta& meta)
{
...
bool CzSMRTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSMRTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSMRTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSMRTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSMRTTracker::GetSerialHashes()
{
...
CAmount CzSMRTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSMRTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSMRTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSMRTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSMRTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSMRTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSMRTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSMRTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSMRTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSMRTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSMRTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSMRTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSMRTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSMRTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSMRTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSMRTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSMRTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
